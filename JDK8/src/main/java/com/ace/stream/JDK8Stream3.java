...
public class JDK8Stream3 {
    ...
    public void test1(){
        ...
        Optional<Double> op1 = emps.stream().map(e->e.getSalary()).reduce((x,y)-> x+y);
    ...
    @Test
    public void test2(){
        Long num = emps.stream().map(e->e.getName()).filter(e-> e.contains("六")).count();
        System.out.println(num);


        Optional<Integer> sum = emps.stream()
                .map(Employee::getName)
                .flatMap(JDK8Stream1::filterCharacter)
                .map((ch) -> {
                    if(ch.equals('六'))
                        return 1;
                    else
